...
export async function getPackageDirectoriesFromGlobs(
  ...
  for (const wsGlob of workspaceGlobs) {
    ...
    workspacesOut = [...workspacesOut, ...entries];
  }

  // Filter out directories containing a package.json
  const tmpOut = await Promise.all(
    workspacesOut.map(async (wsPath) => {
      const tmp = await fs.stat(wsPath);
      if (
        tmp.isDirectory() &&
        (await fs.stat(path.join(wsPath, "package.json")))
      ) {
        return wsPath;
      }
    })
